...
const PROTEIN_RULES: Rule[] = [
  ...
  },
  {
    ...
    keywords: ["ホタテ", "カキ", "貝", "クラム", "ハマグリ", "あさり", "アサリ", "しじみ"],
  },
  {
  ...
  },
  {
    ...
    keywords: ["豆ご飯", "ひじき", "納豆", "枝豆", "大豆", "五目豆"],
  },
  // 動物性タンパク源が1つも取れなかった料理を「野菜メイン」に振り分ける。
  // 他のタンパク源と同時にマッチした場合は下の後処理で取り下げる。
  {
    tagId: "tag-protein-vegetable",
    keywords: [
      "きんぴら",
      "おひたし",
      "サラダ",
...
/** 動物性・豆類のタンパク源タグ。1つでも付いていれば「野菜メイン」は付けない */
const NON_VEGETABLE_PROTEIN_TAG_IDS = new Set(
  PROTEIN_RULES.map((rule) => rule.tagId).filter((id) => id !== "tag-protein-vegetable"),
)
...
function assignTags(dishName: string): string[] {
  ...
  for (const { rules, defaultTagId, allowMultiple } of RULE_SETS) {
    ...
    // 「野菜メイン」は他のタンパク源が取れなかった場合のフォールバック。
    // 例: ピーマンの肉詰め → 合いびき肉のみ、なすの味噌炒め → 野菜メイン
    if (
      matched.includes("tag-protein-vegetable") &&
      matched.some((id) => NON_VEGETABLE_PROTEIN_TAG_IDS.has(id))
    ) {
      tagIds.push(...matched.filter((id) => id !== "tag-protein-vegetable"))
      continue
    }
